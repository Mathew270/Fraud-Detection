...
// KEY CONFIGURATION:
//
// 1. PROXY (/api → sse-stream:8085):
//    When running locally with `npm run dev`, the React app is served on
//    port 5173 but needs to call SSE endpoints on port 8085. Without a proxy,
//    the browser would block these requests due to CORS (different ports =
//    different origins).
//
//    The proxy intercepts any request starting with /api/ and forwards it
//    to http://localhost:8085. The browser thinks it's talking to localhost:5173,
...
export default defineConfig({
  ...
  server: {
    ...
    // calls the SSE backend (port 8085).
    proxy: {
      "/api": {
        // Forward all /api/* requests to the SSE stream backend
